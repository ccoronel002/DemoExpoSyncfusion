var cBlock = ['html-src-tab', 'js-src-tab'];
var switcherPopup;
...
function loadTheme(theme) {
    ...
    var ajax = new ej.base.Ajax(path + '/Content/Style/' + theme + '.css', 'GET', false);
    selectedTheme = theme;
...
function onDataSourceLoad(node, subNode, control, sample, sampleName) {
    ...
    setSbLink();
    var ajaxHTML = new ej.base.Ajax('Chart/Line', 'GET', true);
    var p1 = ajaxHTML.send();
    var ajaxCS = new ej.base.Ajax('Controllers/' + subNode.component + '/' + subNode.url + 'Controller.cs', 'GET', true);
    var ajaxCSHTML = new ej.base.Ajax('Views/Index?path=/Views/' + subNode.component + '/' + subNode.url + '.cshtml', 'GET', true);
    ...
    for (var k = 0; k < 2; k++) {
        ...
        if (header) {
            if (location.pathname.indexOf('/' + subNode.component + '/' + subNode.url) !== -1) {
                header.innerHTML = subNode.url + (k ? 'Controller.cs' : '.cshtml');
            }
        }
    }
    ...
    ajaxCS.send().then(function (value) {
        document.querySelector('.js-source-content').innerHTML = value.replace(/&/g, '&amp;')
    ...
    ajaxCSHTML.send().then(function (value) {
        ...
        document.querySelector('.html-source-content').innerHTML = value.replace(/&/g, '&amp;')
            .replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
